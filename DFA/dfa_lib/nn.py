...
class LinearDFA(torch.nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, error_layer: ErrorDFA, bias: bool = None, continue_BP: bool = False):
        ...
        self.weight = torch.nn.Parameter(torch.Tensor(out_features, in_features))
        self.random_matrix = torch.nn.Parameter(torch.Tensor(self.out_features, out_features))
        self.random_matrix.requires_grad = False
        # self.random_matrix = torch.empty((self.out_features, out_features), dtype=torch.float32, requires_grad=False)
...
class functional():
    ...
    class linearDFA(autograd.Function):
        ...
        def backward(ctx, grad_output: Tensor):
            ...
            d_input = e.mm(weight_fa)*grad_output
